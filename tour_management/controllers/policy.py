...
import json
from ..models import User, Touroperator,Hotel, Room, Event, Cardealer, Policy
...
def add_policy(request):
    ...
    if request.method == 'POST':
        ...
        policy = Policy(tour_operator = touroperator,
                created_by = user,
                name = data['name'],
                type = data['type'],
...
def get_policies(request):
    ...
    if request.method == 'POST':
        ...
        if tour_operator_id is not None and type_id is not None and type is not None:
            policies = Policy.objects.filter(tour_operator =tour_operator_id).filter(type = type).filter(type_id=type_id).all()
        
        for policy in policies:
            user_data = serializers.serialize('json', [policy,])
